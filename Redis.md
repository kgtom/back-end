...
#### 单机部署
 场景：容量或请求不高的小型存储
 部署：
  * 云数据库Redis主从热备模式，一个master，至少一个slave,主从两个哨兵(sentinel)
  * sentinel 作用：监控(所有redis是否正常运行)；故障转移(当master节点发生宕机时，Sentinel会提升 slave 节点为新的 maste)
 缺点：请求多了，性能瓶颈；扩容时只能停机扩容；Sentinel也需要集群搭建，成本高且健康检查不够灵活
#### 集群部署
 场景：当容量超过20G或者系统吞吐量qps超过20w/s时，建立集群。通过中间件(Twemproxy)或客户端分片 将流量分散到各个实例。
 部署：主从redis 至少3个sentinel。
